...
export class DepartamentosService {
  ...
  getDepartamentos(pageIndex = 1, pageSize = 5) {
    ...
    }).pipe(catchError(error => of(error))).subscribe(data => this.departamentos.next(data));
  }

  getDepartamentolist = () =>
    this.http.get<DepartamentoDto[]>(`${this.BaseUrl}/departamentos-list`)

  getDepartamentoByid = (id: number) =>
    this.http.get<Departamentos>(`${this.BaseUrl}/${id}`)
      .pipe(catchError(error => of(error)));


  saveDepartamentos$ = (data: Departamentos) =>
    this.http.post<Departamentos>(`${this.BaseUrl}/departamento`, data)
      .pipe(catchError(error => of(error)));

  updateDepartamentos$ = (data: Departamentos) =>
    this.http.put<Departamentos>(`${this.BaseUrl}/departamento`, data)
      .pipe(catchError(error => of(error)));
}
